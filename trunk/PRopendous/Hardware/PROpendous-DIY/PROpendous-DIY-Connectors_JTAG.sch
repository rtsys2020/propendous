EESchema Schematic File Version 2  date 2/6/2010 1:53:44 AM
LIBS:opendous,power,device,conn,linear,regul,74xx,cmos4000,adc-dac,memory,xilinx,special,microcontrollers,dsp,microchip,analog_switches,motorola,texas,intel,audio,interface,digital-audio,philips,display,cypress,siliconi,opto,atmel,contrib,valves,transistors,.\PROpendous-DIY.cache
EELAYER 24  0
EELAYER END
$Descr A4 11700 8267
Sheet 5 6
Title "Connectors and JTAG"
Date "10 dec 2009"
Rev "1.0"
Comp "Opendous Inc."
Comment1 ""
Comment2 "http://creativecommons.org/licenses/by/3.0/"
Comment3 "Copyright Under the Creative Commons Attribution License"
Comment4 "www.PRopendous.org"
$EndDescr
Text Notes 2425 4975 0    40   ~ 0
PRTR5V0U2X
Text Notes 4210 6725 0    20   ~ 0
(2N7002)
NoConn ~ 9050 2300
Wire Wire Line
	9200 2200 9200 2150
Wire Wire Line
	9200 2150 9100 2150
Wire Wire Line
	9100 2150 9100 2200
Wire Wire Line
	9100 2200 9050 2200
Wire Wire Line
	8100 3900 8100 3850
Wire Wire Line
	8100 3850 8200 3850
Wire Wire Line
	8200 3850 8200 3900
Wire Wire Line
	8200 3900 8250 3900
Wire Wire Line
	9200 5050 9200 5000
Wire Wire Line
	9200 5000 9050 5000
Connection ~ 2050 4450
Wire Wire Line
	2050 4500 2050 4450
Wire Wire Line
	3050 4900 3050 4800
Connection ~ 2700 4150
Wire Wire Line
	2700 4350 2700 4150
Wire Wire Line
	4100 4700 4100 4450
Wire Wire Line
	4100 4450 4150 4450
Wire Wire Line
	1850 4050 4150 4050
Wire Wire Line
	3950 3750 4000 3750
Wire Wire Line
	4150 4250 4000 4250
Wire Wire Line
	4150 4150 1850 4150
Wire Wire Line
	2650 1150 2650 950 
Wire Wire Line
	2650 950  3100 950 
Wire Wire Line
	3200 1250 3200 2050
Wire Wire Line
	3300 1250 3300 1950
Wire Wire Line
	3400 1250 3400 1850
Wire Wire Line
	3500 1250 3500 1750
Wire Wire Line
	3600 1250 3600 1650
Wire Wire Line
	3700 1250 3700 1550
Wire Wire Line
	4475 4850 4475 4800
Wire Wire Line
	2450 6700 2450 6750
Connection ~ 1700 1850
Wire Wire Line
	1800 1850 1700 1850
Wire Wire Line
	2450 2150 4050 2150
Wire Wire Line
	2400 6400 2400 6350
Wire Wire Line
	2400 6350 2350 6350
Wire Wire Line
	2600 6850 2600 6800
Wire Wire Line
	2600 6800 2550 6800
Wire Wire Line
	2800 7300 2800 7250
Wire Wire Line
	2800 7250 2750 7250
Wire Wire Line
	2700 5200 2600 5200
Wire Wire Line
	2900 5200 3000 5200
Wire Wire Line
	4300 6400 4050 6400
Connection ~ 3200 950 
Connection ~ 3400 950 
Wire Wire Line
	3100 950  3100 900 
Wire Wire Line
	3100 900  3300 900 
Connection ~ 3200 2050
Connection ~ 3500 1750
Connection ~ 3700 1550
Wire Wire Line
	2750 1550 4050 1550
Wire Wire Line
	3650 6700 3750 6700
Wire Wire Line
	3000 5200 3000 5300
Wire Wire Line
	1700 1400 1700 1500
Wire Wire Line
	1700 2050 4050 2050
Wire Wire Line
	2750 1850 4050 1850
Wire Wire Line
	2750 1650 4050 1650
Wire Wire Line
	5050 2550 5050 1550
Connection ~ 5050 2350
Connection ~ 5050 2250
Connection ~ 5050 2150
Connection ~ 5050 2050
Connection ~ 5050 1950
Connection ~ 5050 1850
Connection ~ 5050 1750
Connection ~ 5050 1650
Wire Wire Line
	4050 900  4050 1050
Wire Wire Line
	4050 1450 4050 1250
Wire Wire Line
	5050 1450 5050 900 
Wire Wire Line
	2750 1750 4050 1750
Wire Wire Line
	4750 6125 4750 6200
Wire Wire Line
	4050 6900 4050 6800
Wire Wire Line
	4050 950  3400 950 
Connection ~ 4050 950 
Connection ~ 3600 950 
Connection ~ 3700 950 
Connection ~ 3500 950 
Connection ~ 3600 1650
Connection ~ 3400 1850
Wire Wire Line
	3300 1950 4050 1950
Wire Wire Line
	4750 6400 4500 6400
Wire Wire Line
	2650 7150 2650 7200
Wire Wire Line
	2550 7250 2500 7250
Wire Wire Line
	2500 7250 2500 7200
Wire Wire Line
	2350 6800 2300 6800
Wire Wire Line
	2300 6800 2300 6750
Wire Wire Line
	2150 6350 2100 6350
Wire Wire Line
	2100 6350 2100 6300
Wire Wire Line
	2850 1050 2850 850 
Wire Wire Line
	2850 850  3400 850 
Connection ~ 3200 850 
Wire Wire Line
	2850 1250 2850 2150
Connection ~ 2850 2150
Wire Wire Line
	1700 1850 1700 1700
Wire Wire Line
	2250 6250 2250 6300
Wire Wire Line
	4675 4800 4675 4850
Wire Wire Line
	3700 950  3700 1050
Wire Wire Line
	3600 950  3600 1050
Wire Wire Line
	3500 1050 3500 950 
Wire Wire Line
	3400 850  3400 1050
Wire Wire Line
	3300 900  3300 1050
Wire Wire Line
	3200 850  3200 1050
Wire Wire Line
	4150 4350 3950 4350
Wire Wire Line
	3950 4350 3950 4700
Wire Wire Line
	2600 4350 2600 4050
Connection ~ 2600 4050
Wire Wire Line
	1950 4050 1950 4250
Connection ~ 1950 4050
Wire Wire Line
	2250 4800 2250 4700
Wire Wire Line
	4000 4250 4000 3600
Connection ~ 4000 3750
Wire Wire Line
	1950 4450 2150 4450
Wire Wire Line
	2150 4250 2150 4150
Connection ~ 2150 4150
Wire Wire Line
	9050 2100 9200 2100
Wire Wire Line
	8100 2100 8250 2100
Wire Wire Line
	9200 3900 9050 3900
Wire Wire Line
	7950 2200 8250 2200
$Comp
L VSUP4 #PWR058
U 1 1 4B2115B2
P 7950 2200
F 0 "#PWR058" H 7950 2300 30  0001 C CNN
F 1 "VSUP4" H 7950 2300 30  0000 C CNN
	1    7950 2200
	1    0    0    -1  
$EndComp
NoConn ~ 8250 4000
$Comp
L VDD12 #PWR059
U 1 1 4B2068DF
P 8100 2100
F 0 "#PWR059" H 8100 2200 30  0001 C CNN
F 1 "VDD12" H 8100 2200 30  0000 C CNN
	1    8100 2100
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR060
U 1 1 4B2068B2
P 9200 5050
F 0 "#PWR060" H 9200 5050 30  0001 C CNN
F 1 "GND" H 9200 4980 30  0001 C CNN
	1    9200 5050
	1    0    0    -1  
$EndComp
$Comp
L CONN_12X2 P10
U 1 1 4B206637
P 8650 2650
F 0 "P10" H 8650 3300 60  0000 C CNN
F 1 "EXP2" V 8650 2650 50  0000 C CNN
	1    8650 2650
	1    0    0    -1  
$EndComp
$Comp
L CONN_12X2 P11
U 1 1 4B206633
P 8650 4450
F 0 "P11" H 8650 5100 60  0000 C CNN
F 1 "EXP1" V 8650 4450 50  0000 C CNN
	1    8650 4450
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR061
U 1 1 4B2062DE
P 8100 3900
F 0 "#PWR061" H 8100 3900 30  0001 C CNN
F 1 "GND" H 8100 3830 30  0001 C CNN
	1    8100 3900
	1    0    0    -1  
$EndComp
Text GLabel 8250 3100 0    40   BiDi ~ 0
EBI_D_14
Text GLabel 9050 3100 2    40   BiDi ~ 0
EBI_D_12
Text GLabel 9050 2600 2    40   BiDi ~ 0
EBI_D_0
Text GLabel 8250 2600 0    40   BiDi ~ 0
EBI_D_1
Text GLabel 8250 2800 0    40   BiDi ~ 0
EBI_D_7
Text GLabel 9050 2700 2    40   BiDi ~ 0
EBI_D_2
Text GLabel 9050 2800 2    40   BiDi ~ 0
EBI_D_5
Text GLabel 8250 2700 0    40   BiDi ~ 0
EBI_D_3
Text GLabel 8250 2900 0    40   BiDi ~ 0
EBI_D_9
Text GLabel 9050 2900 2    40   BiDi ~ 0
EBI_D_8
$Comp
L GND #PWR062
U 1 1 4B1FCDF0
P 2050 4500
F 0 "#PWR062" H 2050 4500 30  0001 C CNN
F 1 "GND" H 2050 4430 30  0001 C CNN
	1    2050 4500
	1    0    0    -1  
$EndComp
Text Notes 5800 6500 0    50   ~ 0
I2STX_BCK0 and UART_CTS_N are connected together so you must use only one and tri-state the other
$Comp
L USBCONN U2
U 1 1 4B1DBE9E
P 4400 4250
F 0 "U2" H 4450 3950 60  0000 C CNN
F 1 "USB-B" H 4600 4550 40  0000 C CNN
	1    4400 4250
	1    0    0    -1  
$EndComp
$Comp
L GNDPWR #PWR063
U 1 1 4B16D395
P 3050 4900
F 0 "#PWR063" H 3050 4950 40  0001 C CNN
F 1 "GNDPWR" H 3050 4840 25  0000 C CNN
	1    3050 4900
	1    0    0    -1  
$EndComp
Text GLabel 6850 7700 0    30   Input ~ 0
TDO
Text Label 1700 2050 0    40   ~ 0
TDO
Text Label 6850 7700 0    30   ~ 0
TDO
$Comp
L GNDPWR #PWR064
U 1 1 4B16CE0E
P 4100 4700
F 0 "#PWR064" H 4100 4750 40  0001 C CNN
F 1 "GNDPWR" H 4100 4640 25  0000 C CNN
	1    4100 4700
	1    0    0    -1  
$EndComp
Text Notes 7325 4225 0    30   ~ 0
mI2STX_BCK0
$Comp
L VSUP4 #PWR065
U 1 1 4B162FD9
P 9200 3900
F 0 "#PWR065" H 9200 4000 30  0001 C CNN
F 1 "VSUP4" H 9200 4000 30  0000 C CNN
	1    9200 3900
	1    0    0    -1  
$EndComp
$Comp
L MOSFET_N_OPENDOUS Q1
U 1 1 4B08A001
P 3950 6600
F 0 "Q1" H 3950 6790 30  0000 R CNN
F 1 "NCH" H 4350 6600 30  0000 R CNN
	1    3950 6600
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR066
U 1 1 4A6C0AB3
P 9200 2200
F 0 "#PWR066" H 9200 2200 30  0001 C CNN
F 1 "GND" H 9200 2130 30  0001 C CNN
	1    9200 2200
	1    0    0    -1  
$EndComp
NoConn ~ 4050 2250
NoConn ~ 4050 2350
$Comp
L R_MINI R81
U 1 1 4A6BA953
P 3600 1150
F 0 "R81" V 3575 1200 25  0000 C CNN
F 1 "10k" V 3625 1200 20  0000 C CNN
	1    3600 1150
	0    1    1    0   
$EndComp
$Comp
L R_MINI R80
U 1 1 4A6BA952
P 3500 1150
F 0 "R80" V 3475 1200 25  0000 C CNN
F 1 "10k" V 3525 1200 20  0000 C CNN
	1    3500 1150
	0    1    1    0   
$EndComp
$Comp
L R_MINI R82
U 1 1 4A6BA951
P 3700 1150
F 0 "R82" V 3675 1200 25  0000 C CNN
F 1 "10k" V 3725 1200 20  0000 C CNN
	1    3700 1150
	0    1    1    0   
$EndComp
$Comp
L R_MINI R78
U 1 1 4A6BA94F
P 3300 1150
F 0 "R78" V 3275 1200 25  0000 C CNN
F 1 "10k" V 3325 1200 20  0000 C CNN
	1    3300 1150
	0    1    1    0   
$EndComp
$Comp
L R_MINI R79
U 1 1 4A6BA94E
P 3400 1150
F 0 "R79" V 3375 1200 25  0000 C CNN
F 1 "10k" V 3425 1200 20  0000 C CNN
	1    3400 1150
	0    1    1    0   
$EndComp
$Comp
L R_MINI R77
U 1 1 4A6BA94B
P 3200 1150
F 0 "R77" V 3175 1200 25  0000 C CNN
F 1 "10k" V 3225 1200 20  0000 C CNN
	1    3200 1150
	0    1    1    0   
$EndComp
$Comp
L VBUS #PWR067
U 1 1 4A6ABDF2
P 9200 2100
F 0 "#PWR067" H 9200 2200 30  0001 C CNN
F 1 "VBUS" H 9200 2200 30  0000 C CNN
	1    9200 2100
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR068
U 1 1 4A6AB40F
P 4675 4850
F 0 "#PWR068" H 4675 4850 30  0001 C CNN
F 1 "GND" H 4675 4780 30  0001 C CNN
	1    4675 4850
	1    0    0    -1  
$EndComp
$Comp
L GNDPWR #PWR069
U 1 1 4A6AB40B
P 4475 4850
F 0 "#PWR069" H 4475 4900 40  0001 C CNN
F 1 "GNDPWR" H 4475 4790 25  0000 C CNN
	1    4475 4850
	1    0    0    -1  
$EndComp
$Comp
L L_MINI L10
U 1 1 4A6AB403
P 4575 4800
F 0 "L10" H 4500 4750 30  0000 C CNN
F 1 "FB" H 4625 4750 22  0000 C CNN
	1    4575 4800
	-1   0    0    1   
$EndComp
$Comp
L R_MINI R76
U 1 1 4A64F6EA
P 1700 1600
F 0 "R76" V 1675 1650 25  0000 C CNN
F 1 "4k7" V 1725 1650 20  0000 C CNN
	1    1700 1600
	0    1    1    0   
$EndComp
Text Notes 1700 6600 0    30   Italic 0
(Default)
$Comp
L VSUP3 #PWR070
U 1 1 4A5BB11D
P 2500 7200
F 0 "#PWR070" H 2500 7300 30  0001 C CNN
F 1 "VSUP3" H 2500 7300 30  0000 C CNN
	1    2500 7200
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR071
U 1 1 4A5BB11A
P 2800 7300
F 0 "#PWR071" H 2800 7300 30  0001 C CNN
F 1 "GND" H 2800 7230 30  0001 C CNN
	1    2800 7300
	1    0    0    -1  
$EndComp
$Comp
L VSUP3 #PWR072
U 1 1 4A5BB116
P 2300 6750
F 0 "#PWR072" H 2300 6850 30  0001 C CNN
F 1 "VSUP3" H 2300 6850 30  0000 C CNN
	1    2300 6750
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR073
U 1 1 4A5BB113
P 2600 6850
F 0 "#PWR073" H 2600 6850 30  0001 C CNN
F 1 "GND" H 2600 6780 30  0001 C CNN
	1    2600 6850
	1    0    0    -1  
$EndComp
$Comp
L VSUP3 #PWR074
U 1 1 4A5BB10C
P 2100 6300
F 0 "#PWR074" H 2100 6400 30  0001 C CNN
F 1 "VSUP3" H 2100 6400 30  0000 C CNN
	1    2100 6300
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR075
U 1 1 4A5BB100
P 2400 6400
F 0 "#PWR075" H 2400 6400 30  0001 C CNN
F 1 "GND" H 2400 6330 30  0001 C CNN
	1    2400 6400
	1    0    0    -1  
$EndComp
$Comp
L R_MINI R74
U 1 1 4A5BB0EF
P 2250 6150
F 0 "R74" V 2225 6200 25  0000 C CNN
F 1 "4k7" V 2275 6200 20  0000 C CNN
	1    2250 6150
	0    1    1    0   
$EndComp
$Comp
L R_MINI R75
U 1 1 4A5BB0ED
P 2450 6600
F 0 "R75" V 2425 6650 25  0000 C CNN
F 1 "4k7" V 2475 6650 20  0000 C CNN
	1    2450 6600
	0    1    1    0   
$EndComp
$Comp
L R_MINI R73
U 1 1 4A5BB0CF
P 2650 7050
F 0 "R73" V 2625 7100 25  0000 C CNN
F 1 "4k7" V 2675 7100 20  0000 C CNN
	1    2650 7050
	0    1    1    0   
$EndComp
$Comp
L JUMPER-3 JP0
U 1 1 4A5BB068
P 2250 6350
F 0 "JP0" H 2250 6290 30  0000 C CNN
F 1 "JUMPER-3" H 2250 6350 30  0001 C CNN
	1    2250 6350
	1    0    0    -1  
$EndComp
$Comp
L JUMPER-3 JP2
U 1 1 4A5BB064
P 2650 7250
F 0 "JP2" H 2650 7190 30  0000 C CNN
F 1 "JUMPER-3" H 2650 7250 30  0001 C CNN
	1    2650 7250
	-1   0    0    -1  
$EndComp
$Comp
L JUMPER-3 JP1
U 1 1 4A5BB05B
P 2450 6800
F 0 "JP1" H 2450 6740 30  0000 C CNN
F 1 "JUMPER-3" H 2450 6800 30  0001 C CNN
	1    2450 6800
	1    0    0    -1  
$EndComp
$Comp
L R_MINI R26
U 1 1 49F7CD20
P 2800 5200
AR Path="/4A0869EC/49F7CD20" Ref="R26"  Part="1" 
AR Path="/49F49582/49F7CD20" Ref="R26"  Part="1" 
F 0 "R26" H 2730 5250 25  0000 C CNN
F 1 "12k" H 2870 5250 20  0000 C CNN
	1    2800 5200
	1    0    0    -1  
$EndComp
Text Notes 2765 5280 0    30   ~ 0
1%
Text GLabel 2650 6950 0    40   BiDi ~ 0
GPIO2
$Comp
L GND #PWR0292
U 1 1 49FB6945
P 2650 1150
AR Path="/4A0869EC/49FB6945" Ref="#PWR0292"  Part="1" 
AR Path="/49F49582/49FB6945" Ref="#PWR076"  Part="1" 
F 0 "#PWR076" H 2650 1150 30  0001 C CNN
F 1 "GND" H 2650 1080 30  0001 C CNN
	1    2650 1150
	1    0    0    -1  
$EndComp
Text Notes 3750 6050 0    40   ~ 0
Blink on Error
Text Notes 3700 5850 0    60   ~ 0
Boot Status
$Comp
L GND #PWR0293
U 1 1 49FB5FDA
P 4050 6900
AR Path="/4A0869EC/49FB5FDA" Ref="#PWR0293"  Part="1" 
AR Path="/49F49582/49FB5FDA" Ref="#PWR077"  Part="1" 
F 0 "#PWR077" H 4050 6900 30  0001 C CNN
F 1 "GND" H 4050 6830 30  0001 C CNN
	1    4050 6900
	1    0    0    -1  
$EndComp
Text Notes 4210 6675 0    20   ~ 0
(SOT-23)
$Comp
L VSUP3 #PWR0294
U 1 1 49FB5F5C
P 4750 6125
AR Path="/4A0869EC/49FB5F5C" Ref="#PWR0294"  Part="1" 
AR Path="/49F49582/49FB5F5C" Ref="#PWR078"  Part="1" 
F 0 "#PWR078" H 4750 6225 30  0001 C CNN
F 1 "VSUP3" H 4750 6225 30  0000 C CNN
	1    4750 6125
	1    0    0    -1  
$EndComp
$Comp
L R_MINI R33
U 1 1 49FB5F38
P 4750 6300
AR Path="/4A0869EC/49FB5F38" Ref="R33"  Part="1" 
AR Path="/49F49582/49FB5F38" Ref="R33"  Part="1" 
F 0 "R33" V 4725 6350 25  0000 C CNN
F 1 "1k" V 4775 6350 20  0000 C CNN
	1    4750 6300
	0    1    1    0   
$EndComp
$Comp
L D_LED D2
U 1 1 49FB5E87
P 4400 6400
AR Path="/4A0869EC/49FB5E87" Ref="D2"  Part="1" 
AR Path="/49F49582/49FB5E87" Ref="D2"  Part="1" 
F 0 "D2" H 4400 6500 50  0000 C CNN
F 1 "LED" H 4400 6285 50  0000 C CNN
	1    4400 6400
	-1   0    0    1   
$EndComp
Text GLabel 3650 6700 0    40   BiDi ~ 0
GPIO2
Text Notes 8000 1750 0    60   ~ 0
Expansion Connectors
$Comp
L GND #PWR0298
U 1 1 49F7CC82
P 3000 5300
AR Path="/4A0869EC/49F7CC82" Ref="#PWR0298"  Part="1" 
AR Path="/49F49582/49F7CC82" Ref="#PWR079"  Part="1" 
F 0 "#PWR079" H 3000 5300 30  0001 C CNN
F 1 "GND" H 3000 5230 30  0001 C CNN
	1    3000 5300
	1    0    0    -1  
$EndComp
Text Notes 1300 6375 0    30   Italic 0
GPIO Pin
Text Notes 3200 700  0    60   ~ 0
JTAG
Text GLabel 1800 1850 2    40   BiDi ~ 0
JTAGSEL
$Comp
L VSUP3 #PWR0302
U 1 1 49F7BECC
P 1700 1400
AR Path="/4A0869EC/49F7BECC" Ref="#PWR0302"  Part="1" 
AR Path="/49F49582/49F7BECC" Ref="#PWR080"  Part="1" 
F 0 "#PWR080" H 1700 1500 30  0001 C CNN
F 1 "VSUP3" H 1700 1500 30  0000 C CNN
	1    1700 1400
	1    0    0    -1  
$EndComp
$Comp
L R_MINI R27
U 1 1 49F7BB94
P 2850 1150
AR Path="/4A0869EC/49F7BB94" Ref="R27"  Part="1" 
AR Path="/49F49582/49F7BB94" Ref="R27"  Part="1" 
F 0 "R27" V 2825 1200 25  0000 C CNN
F 1 "10k" V 2875 1200 20  0000 C CNN
	1    2850 1150
	0    1    1    0   
$EndComp
$Comp
L GND #PWR0304
U 1 1 49F7B93D
P 5050 2550
AR Path="/4A0869EC/49F7B93D" Ref="#PWR0304"  Part="1" 
AR Path="/49F49582/49F7B93D" Ref="#PWR081"  Part="1" 
F 0 "#PWR081" H 5050 2550 30  0001 C CNN
F 1 "GND" H 5050 2480 30  0001 C CNN
	1    5050 2550
	1    0    0    -1  
$EndComp
$Comp
L VSUP3 #PWR0305
U 1 1 49F7B91C
P 5050 900
AR Path="/4A0869EC/49F7B91C" Ref="#PWR0305"  Part="1" 
AR Path="/49F49582/49F7B91C" Ref="#PWR082"  Part="1" 
F 0 "#PWR082" H 5050 1000 30  0001 C CNN
F 1 "VSUP3" H 5050 1000 30  0000 C CNN
	1    5050 900 
	1    0    0    -1  
$EndComp
$Comp
L VSUP3 #PWR0306
U 1 1 49F7B912
P 4050 900
AR Path="/4A0869EC/49F7B912" Ref="#PWR0306"  Part="1" 
AR Path="/49F49582/49F7B912" Ref="#PWR083"  Part="1" 
F 0 "#PWR083" H 4050 1000 30  0001 C CNN
F 1 "VSUP3" H 4050 1000 30  0000 C CNN
	1    4050 900 
	1    0    0    -1  
$EndComp
$Comp
L R_MINI R29
U 1 1 49F7B8F4
P 4050 1150
AR Path="/4A0869EC/49F7B8F4" Ref="R29"  Part="1" 
AR Path="/49F49582/49F7B8F4" Ref="R29"  Part="1" 
F 0 "R29" V 4025 1200 25  0000 C CNN
F 1 "33" V 4075 1200 20  0000 C CNN
	1    4050 1150
	0    1    1    0   
$EndComp
$Comp
L CONN_JTAG_ARM U4
U 1 1 49F7B888
P 4550 1900
AR Path="/4A0869EC/49F7B888" Ref="U4"  Part="1" 
AR Path="/49F49582/49F7B888" Ref="U4"  Part="1" 
F 0 "U4" H 4350 1350 50  0000 C CNN
F 1 "JTAG" H 4550 2450 50  0000 C CNN
	1    4550 1900
	1    0    0    -1  
$EndComp
Text GLabel 9050 2500 2    40   BiDi ~ 0
mLCD_DB_8-EBI_A8
Text GLabel 8250 2500 0    40   BiDi ~ 0
mLCD_DB_7-EBI_A7
Text GLabel 8250 2400 0    40   BiDi ~ 0
mLCD_DB_5-EBI_A5
Text GLabel 9050 2400 2    40   BiDi ~ 0
mLCD_DB_4-EBI_A4
Text GLabel 8250 2300 0    40   BiDi ~ 0
mLCD_DB_3-EBI_A3
Text GLabel 9050 4000 2    40   BiDi ~ 0
UART_TXD
Text GLabel 8250 4100 0    40   BiDi ~ 0
UART_RXD
Text GLabel 9050 4100 2    40   BiDi ~ 0
mUART_RTS_N
Text GLabel 8250 4200 0    40   BiDi ~ 0
mUART_CTS_N
Text GLabel 8250 4900 0    40   BiDi ~ 0
SPI_MOSI
Text GLabel 9050 4800 2    40   BiDi ~ 0
SPI_MISO
Text GLabel 9050 4900 2    40   BiDi ~ 0
SPI_SCK
Text GLabel 8250 4700 0    40   BiDi ~ 0
ADC10B_GPA3
Text GLabel 8250 4800 0    40   BiDi ~ 0
ADC10B_GPA1
Text GLabel 9050 4700 2    40   BiDi ~ 0
ADC10B_GPA0
Text GLabel 2600 5200 0    40   BiDi ~ 0
USB_RREF
Text GLabel 1700 2050 0    40   BiDi ~ 0
ARM_TDO
Text GLabel 2750 1750 0    40   BiDi ~ 0
TMS
Text GLabel 2750 1850 0    40   BiDi ~ 0
TCK
Text GLabel 2750 1550 0    40   BiDi ~ 0
TRST_N
Text GLabel 2750 1650 0    40   BiDi ~ 0
TDI
Text GLabel 9050 4300 2    40   BiDi ~ 0
CLK_256FS_O
Text Notes 1950 2250 0    30   ~ 0
Debugger can RESET board
Text Notes 1650 2650 0    40   ~ 0
Note: JTAGSEL (JTAG TDO Mux) pulled up to 3V sets ARM-DEBUG Mode
Text Notes 700  6750 0    35   ~ 0
Reserved
Text Notes 1400 6900 0    30   ~ 0
1
Text Notes 1400 6850 0    30   ~ 0
1
Text Notes 1600 6850 0    30   ~ 0
0
Text Notes 1600 6800 0    30   ~ 0
1
Text Notes 1400 6800 0    30   ~ 0
0
Text Notes 1600 6750 0    30   ~ 0
0
Text Notes 1600 6900 0    30   ~ 0
1
Text Notes 1400 6750 0    30   ~ 0
0
Text Notes 1400 6700 0    30   ~ 0
1
Text Notes 1400 6650 0    30   ~ 0
1
Text Notes 1600 6650 0    30   ~ 0
0
Text Notes 1600 6600 0    30   ~ 0
1
Text Notes 1200 6900 0    30   ~ 0
1
Text Notes 1200 6850 0    30   ~ 0
1
Text Notes 1200 6700 0    30   ~ 0
0
Text Notes 1200 6650 0    30   ~ 0
0
Text Notes 1200 6600 0    30   ~ 0
0
Text Notes 1400 6600 0    30   ~ 0
0
Text Notes 1200 6550 0    30   ~ 0
0
Text Notes 1600 6550 0    30   ~ 0
0
Text Notes 1600 6700 0    30   ~ 0
1
Text Notes 1200 6800 0    30   ~ 0
1
Text Notes 1400 6550 0    30   ~ 0
0
Text Notes 1200 6750 0    30   ~ 0
1
Text Notes 875  6900 0    35   ~ 0
Test
Text Notes 875  6850 0    35   ~ 0
UART
Text Notes 900  6800 0    35   ~ 0
NOR
Text Notes 900  6700 0    35   ~ 0
MMC
Text Notes 900  6650 0    35   ~ 0
DFU
Text Notes 750  6600 0    35   ~ 0
SPI NOR
Text Notes 875  6550 0    35   ~ 0
NAND
Text Notes 1600 6450 0    40   Italic 0
2
Text Notes 1400 6450 0    40   Italic 0
1
Text Notes 1200 6450 0    40   Italic 0
0
Text Notes 550  6450 0    40   Italic 0
Boot Device
Text Notes 800  6300 0    40   ~ 0
Boot Mode Select
Text Notes 1050 5850 0    60   ~ 0
Mode Select Jumpers
Text GLabel 2450 2150 0    40   BiDi ~ 0
^JTAG_SRST
Text GLabel 2450 6500 0    40   BiDi ~ 0
GPIO1
Text GLabel 2250 6050 0    40   BiDi ~ 0
GPIO0
Text GLabel 8250 4400 0    40   BiDi ~ 0
GPIO14
Text GLabel 8250 3200 0    40   BiDi ~ 0
mNAND_RYBN2-MCI_DAT6
Text GLabel 8250 5000 0    40   BiDi ~ 0
RESET_SUP3
Text GLabel 9050 3000 2    40   BiDi ~ 0
EBI_D_10
Text GLabel 8250 3000 0    40   BiDi ~ 0
EBI_D_11
Text GLabel 9050 3200 2    40   BiDi ~ 0
MCI_DAT3
Text GLabel 8250 4600 0    40   BiDi ~ 0
I2C_SCL1
Text GLabel 9050 4600 2    40   BiDi ~ 0
I2C_SDA1
Text GLabel 9050 4200 2    40   BiDi ~ 0
mI2STX_WS0
Text GLabel 8250 4300 0    40   BiDi ~ 0
mI2STX_DATA0
Text GLabel 9050 4500 2    40   BiDi ~ 0
I2SRX_WS1
Text GLabel 8250 4500 0    40   BiDi ~ 0
I2SRX_BCK1
Text GLabel 9050 4400 2    40   BiDi ~ 0
I2SRX_DATA1
$Comp
L VBUS #PWR0291
U 1 1 49FE2E23
P 2250 4700
AR Path="/4A0869EC/49FE2E23" Ref="#PWR0291"  Part="1" 
AR Path="/49F49582/49FE2E23" Ref="#PWR084"  Part="1" 
F 0 "#PWR084" H 2250 4800 30  0001 C CNN
F 1 "VBUS" H 2250 4800 30  0000 C CNN
	1    2250 4700
	1    0    0    -1  
$EndComp
$Comp
L PRTR5V0U2X IC16
U 1 1 49FE2DAD
P 2650 4600
AR Path="/4A0869EC/49FE2DAD" Ref="IC16"  Part="1" 
AR Path="/49F49582/49FE2DAD" Ref="IC21"  Part="1" 
F 0 "IC21" H 2475 4675 35  0000 C CNN
F 1 "PRTR" H 2650 4315 35  0000 C CNN
	1    2650 4600
	1    0    0    -1  
$EndComp
Text Notes 3150 3350 0    60   ~ 0
USB
$Comp
L R_MINI R24
U 1 1 49F7CD99
P 1950 4350
AR Path="/4A0869EC/49F7CD99" Ref="R24"  Part="1" 
AR Path="/49F49582/49F7CD99" Ref="R24"  Part="1" 
F 0 "R24" V 1925 4400 25  0000 C CNN
F 1 "1M" V 1975 4400 20  0000 C CNN
	1    1950 4350
	0    1    1    0   
$EndComp
$Comp
L R_MINI R25
U 1 1 49F7CD63
P 2150 4350
AR Path="/4A0869EC/49F7CD63" Ref="R25"  Part="1" 
AR Path="/49F49582/49F7CD63" Ref="R25"  Part="1" 
F 0 "R25" V 2125 4400 25  0000 C CNN
F 1 "1M" V 2175 4400 20  0000 C CNN
	1    2150 4350
	0    1    1    0   
$EndComp
$Comp
L VBUS #PWR0297
U 1 1 49F7CCA8
P 4000 3600
AR Path="/4A0869EC/49F7CCA8" Ref="#PWR0297"  Part="1" 
AR Path="/49F49582/49F7CCA8" Ref="#PWR085"  Part="1" 
F 0 "#PWR085" H 4000 3700 30  0001 C CNN
F 1 "VBUS" H 4000 3700 30  0000 C CNN
	1    4000 3600
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR0298
U 1 1 49F7CC82
P 3950 4700
AR Path="/4A0869EC/49F7CC82" Ref="#PWR0298"  Part="1" 
AR Path="/49F49582/49F7CC82" Ref="#PWR086"  Part="1" 
F 0 "#PWR086" H 3950 4700 30  0001 C CNN
F 1 "GND" H 3950 4630 30  0001 C CNN
	1    3950 4700
	1    0    0    -1  
$EndComp
Text GLabel 1850 4150 0    40   BiDi ~ 0
USB_DM
Text GLabel 1850 4050 0    40   BiDi ~ 0
USB_DP
Text GLabel 3950 3750 0    40   BiDi ~ 0
USB_VBUS
$EndSCHEMATC
